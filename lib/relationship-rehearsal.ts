...
export type RelationshipRehearsal = { opening: string; perspective: string; checkpoints: string[]; paths: RehearsalPath[] };
...
export function createRelationshipRehearsal(person: PrivatePerson, situation: string, need: string): RelationshipRehearsal {
  ...
  return {
    ...
    checkpoints: ["先说一个具体场景，再说你的感受；避免用“你总是／你从不”开头。", "把“希望你改变”换成一个可以回应的具体请求。", "给对方停顿和不同意见的空间，不把一次谈话当成关系的最终判决。"],
    paths: [
