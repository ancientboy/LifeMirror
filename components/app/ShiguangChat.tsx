...
export function ShiguangChat({ theme, context, opening = "如果你对这次结果还有疑问，可以继续问我。我们一起把象征放回真实生活里。", mode = "result" }: Props) {
  ...
  const quickPrompts = mode === "home" ? ["我最近总在为一件事焦虑", "有件事让我一直放不下", "我想听听你的判断"] : theme === "east" ? ["这次最该留意什么？", "这层关系接下来该怎么看？", "直接告诉我你的判断"] : ["哪张牌最关键？", "为什么会这样解释？", "直接告诉我你的判断"];
  ...
  return <section className={`${styles.chat} ${styles[theme]} ${styles[mode]}`} aria-label="继续和拾光聊聊">
    ...
    <footer>{savedNotice ? "已记下这件事；你可以随时在“我的”中删除。" : temporary ? "临时对话不会留在记录里。" : responseMode === "local" ? "这段对话已保存。" : memorySettings.enabled ? "这段对话已保存；拾光会在相关时想起你授权保留的线索。" : "这段对话已保存。"}</footer>
  </section>;
}
