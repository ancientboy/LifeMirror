...
import { createRelationshipLoop, deletePersonObservation, getPrivatePeople, getRelationshipLoopsForPerson, savePersonObservation, savePrivatePerson, type PrivatePerson } from "@/lib/relationship-context";
...
export function PersonMirror({ person, onClose, onPractice }: { person: PrivatePerson; onClose: () => void; onPractice: (person: PrivatePerson) => void }) {
  ...
  return <div className={styles.backdrop}><main className={styles.page}>
    ...
    <section className={styles.grid}><article><small>拾光目前看到的线索</small><h2>{personMirrorInsight(context)}</h2><p>来自你的 {context.ownerObservations.length} 条观察、{context.realInteractions.length} 次真实反馈和 {context.simulationCorrections.length} 条模拟纠正。它们不是对 TA 的人格结论。</p></article><article><small>未结束的事</small><h2>{context.openLoops.length ? `有 ${context.openLoops.length} 次沟通正等你带回结果` : "暂时没有等待中的沟通"}</h2><p>现实里的回应会成为下一次练习最优先的线索。</p></article></section>
    <section className={styles.timeline}><header><span><small>TA 的镜像资料</small><h2>只保留你的视角</h2></span><button type="button" onClick={() => setProfileOpen((value) => !value)}>{profileOpen ? "收起" : "完善资料"}</button></header><p>{currentPerson.userDescription || "目前还没有你的观察。可以从一件最近发生的具体事开始。"}</p>{currentPerson.communicationNotes && <p>沟通时你想留意：{currentPerson.communicationNotes}</p>}{currentPerson.birthProfile && <p>出生底图：{currentPerson.birthProfile.date}{currentPerson.birthProfile.timeKnown ? ` · ${currentPerson.birthProfile.time}` : " · 时间未知"}{currentPerson.birthProfile.place ? ` · ${currentPerson.birthProfile.place}` : ""}。它只作为待现实验证的象征参考，不会成为人格结论。</p>}{currentPerson.isMinor && <p>未成年人保护已启用：此镜像仅作为你的私密沟通准备，不提供邀请或共享入口。</p>}{profileOpen && <form onSubmit={(event) => { event.preventDefault(); void updateProfile(new FormData(event.currentTarget)); }}><input name="displayName" defaultValue={currentPerson.displayName} required maxLength={40} aria-label="TA 的昵称"/><input name="relationshipType" defaultValue={currentPerson.relationshipType} maxLength={40} placeholder="和我的关系"/><textarea name="description" defaultValue={currentPerson.userDescription} maxLength={300} placeholder="TA 在你心里是什么样的人？"/><textarea name="communicationNotes" defaultValue={currentPerson.communicationNotes} maxLength={300} placeholder="什么事最容易让 TA 防御或卡住？"/><label>出生日期（可选）<input name="birthDate" type="date" defaultValue={currentPerson.birthProfile?.date}/></label><label>出生时间（未知可留空）<input name="birthTime" type="time" defaultValue={currentPerson.birthProfile?.time}/></label><input name="birthPlace" defaultValue={currentPerson.birthProfile?.place} maxLength={80} placeholder="出生地（可选）"/><label>UTC 偏移（分钟）<input name="utcOffsetMinutes" type="number" defaultValue={currentPerson.birthProfile?.utcOffsetMinutes} placeholder="480"/></label><label>纬度<input name="latitude" type="number" step="0.0001" defaultValue={currentPerson.birthProfile?.latitude}/></label><label>经度<input name="longitude" type="number" step="0.0001" defaultValue={currentPerson.birthProfile?.longitude}/></label><label><input name="isMinor" type="checkbox" defaultChecked={currentPerson.isMinor}/> TA 是未成年人（仅私密沟通准备）</label><p>填写地点、时区与经纬度后，会复用八字与星盘引擎建立可去重的出生底图；时间未知时不会伪造时柱、上升或宫位。</p><button>保存你的观察</button></form>}</section>
    <section className={styles.chat}><header><div><small>和 {currentPerson.displayName} 练习一下</small><h2>说一句你真的可能会说的话</h2></div><ChatCenteredText /></header><p>这里是一种可纠正的可能回应，不是现实中的 TA，也不会被自动写成事实。</p><div className={styles.messages}>{messages.map((message) => <article className={message.role === "user" ? styles.user : styles.simulation} key={message.id}><b>{message.role === "user" ? "你" : `${currentPerson.displayName} · 模拟`}</b><p>{message.text}</p>{message.role === "simulation" && <div><button onClick={() => setCorrection(null)}>像 TA</button><button onClick={() => setCorrection(null)}>有一点像</button><button onClick={() => setCorrection(message.id)}>不像 TA</button></div>}{correction === message.id && <aside><input autoFocus value={correctionText} onChange={(event) => setCorrectionText(event.target.value)} placeholder="TA 更可能怎么说？"/><button onClick={saveCorrection}>保存这条纠正</button></aside>}</article>)}</div><div className={styles.compose}><textarea value={draft} onChange={(event) => setDraft(event.target.value)} placeholder={`例如：${currentPerson.displayName}，我想聊聊昨天那件事……`} maxLength={240}/><button onClick={send}>发送</button></div>{messages.length > 0 && <><p className={styles.coach}><Sparkle />{coachRehearsalReply(messages.filter((item) => item.role === "user").at(-1)?.text ?? "")}</p><button className={styles.reality} onClick={prepareReality} disabled={loopCreated}><Check />{loopCreated ? "已准备，之后回来告诉拾光结果" : "我准备这么说，之后回来告诉拾光结果"}</button></>}</section>
