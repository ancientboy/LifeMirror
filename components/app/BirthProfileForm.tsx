...
import { calculateAstrology, calculateAstrologyTransits, calculateSecondaryProgressions } from "../../server/tools/astrology/core";
  ...
  async function submit(event: React.FormEvent) {
    ...
    try {
      ...
      if (profileOnly) {
        ...
        const birthInput = { year, month, day: validDay, hour: unknownTime ? null : hour, minute: unknownTime ? 0 : minute, utcOffsetMinutes, timeZone, latitude: Number(formatCoordinate(Number(latitude))), longitude: Number(formatCoordinate(Number(longitude))) };
        const natal = calculateAstrology(birthInput);
        const now = new Date();
        const targetInput = { ...birthInput, year: now.getUTCFullYear(), month: now.getUTCMonth() + 1, day: now.getUTCDate(), hour: 12, minute: 0 };
        nextResult = { ...natal, timing: { calculatedFor: now.toISOString().slice(0, 10), transits: calculateAstrologyTransits(natal, targetInput), progressions: calculateSecondaryProgressions(natal, birthInput, targetInput) } };
...
function BaziChart({ result, savedReflection, historical, onReflection }: { result: BaziResult; savedReflection?: MirrorResult; historical: boolean; onReflection: (reflection: MirrorResult) => void }) {
  ...
  return <section className={styles.chart} aria-live="polite">
    ...
    <section className={styles.analysisSection}><div className={styles.sectionHeading}><div><small>LUCK CYCLES · 大运流年流月</small><h3>{result.luck ? `${result.luck.direction} · 起运约 ${result.luck.startsAfter}` : "尚未生成排运序列"}</h3></div></div>{result.luck ? <><div className={styles.luckCycles}>{result.luck.cycles.map((cycle) => <article key={`${cycle.ganZhi}-${cycle.startYear}`}><strong>{cycle.ganZhi}</strong><span>{cycle.startYear}–{cycle.endYear}</span><small>{cycle.startAge}–{cycle.endAge} 岁</small></article>)}</div><div className={styles.annuals}>{result.luck.annual.map((item) => <article className={item.year === currentYear ? styles.currentAnnual : ""} key={item.year}><b>{item.year}</b><strong>{item.ganZhi}</strong><span>{item.tenGod} · {item.age} 岁</span></article>)}</div>{result.luck.monthly.length > 0 && <><p className={styles.methodNote}>{currentYear} 节令流月 · 仅展示传统结构，不直接断吉凶</p><div className={styles.annuals}>{result.luck.monthly.map((item) => <article key={`${item.month}-${item.ganZhi}`}><b>{item.month}</b><strong>{item.ganZhi}</strong><span>{item.tenGod}{item.branchRelations.length ? ` · ${item.branchRelations.map((relation) => `${relation.natalBranch}${relation.kind}`).join("、")}` : ""}</span></article>)}</div></>}<p className={styles.methodNote}>{result.luck.method}</p></> : <p className={styles.emptyAnalysis}>请选择传统排运参数并提供准确出生时间，系统才会计算起运、大运、流年与流月；缺少条件时不会猜测。</p>}</section>
