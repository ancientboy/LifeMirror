...
function BaziChart({ result }: { result: BaziResult }) {
  ...
  return <section className={styles.chart} aria-live="polite">
    <header><div><small>FOUR PILLARS MIRROR</small><h2>你的四柱命盘</h2></div></header>
    ...
    <details><summary>查看专业推导依据</summary><div className={styles.evidence}><article><h3>计算时间</h3><p>{result.effectiveLocalTime}{result.trueSolarAdjustmentMinutes !== null && `（真太阳时修正 ${result.trueSolarAdjustmentMinutes >= 0 ? "+" : ""}${result.trueSolarAdjustmentMinutes} 分钟）`}</p><p>引擎 {result.engine.version} · 支持历法范围：{result.engine.calendarRange}</p></article><article><h3>相邻节气</h3><p>{result.solarTerms.previous} · {result.solarTerms.previousAt}</p><p>{result.solarTerms.next} · {result.solarTerms.nextAt}</p></article></div><ul>{result.rules.map((item) => <li key={item}>{item}</li>)}</ul><ul className={styles.warnings}>{result.warnings.map((item) => <li key={item}>{item}</li>)}</ul></details>
  </section>;
}
