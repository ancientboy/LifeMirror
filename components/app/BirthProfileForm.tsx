...
function BaziChart({ result, savedReflection, historical, onReflection }: { result: BaziResult; savedReflection?: MirrorResult; historical: boolean; onReflection: (reflection: MirrorResult) => void }) {
  ...
  return <section className={styles.chart} aria-live="polite">
    ...
    <div className={styles.evidence}><article><h3>本次排盘时间</h3><p>{result.effectiveLocalTime}{result.trueSolarAdjustmentMinutes !== null && `（已按真太阳时校正 ${result.trueSolarAdjustmentMinutes >= 0 ? "+" : ""}${result.trueSolarAdjustmentMinutes} 分钟）`}</p></article><article><h3>节气位置</h3><p>上一节气：{result.solarTerms.previous}</p><p>下一节气：{result.solarTerms.next}</p></article></div>{result.warnings.filter((item) => item.includes("出生时间未知") || item.includes("真太阳时")).map((item) => <p className={styles.methodNote} key={item}>{item}</p>)}
    </details>
