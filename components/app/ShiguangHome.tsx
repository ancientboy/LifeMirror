...
export function ShiguangHome() {
  ...
  useEffect(() => {
    try {
      const history = JSON.parse(window.localStorage.getItem("life-mirror:guest-history:v1") ?? "[]") as Array<{ question?: string; savedAt?: string; reflection?: { shareableReflection?: string; shiguangInterpretation?: string; traditionalJudgment?: string } }>;
