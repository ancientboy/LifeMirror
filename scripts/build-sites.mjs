...
function relationshipClassification(text, label = "") {
  const value = (String(label || "") + " " + String(text || "")).toLowerCase();
  let domain = "other", role = "other", powerPosition = "roughly_equal";
  if (/暧昧|约会|dating|相亲|crush|追求|喜欢的人/u.test(value)) { domain = "romance"; role = "dating"; }
  else if (/男友|女友|对象|伴侣|老公|老婆|丈夫|妻子|partner/u.test(value)) { domain = "romance"; role = "partner"; }
  else if (/前任|前男友|前女友|ex\b/u.test(value)) { domain = "romance"; role = "ex"; }
  else if (/领导|上司|老板|主管|manager|boss/u.test(value)) { domain = "work"; role = "manager"; powerPosition = "user_lower_power"; }
  else if (/下属|员工|汇报给我|report\b/u.test(value)) { domain = "work"; role = "report"; powerPosition = "user_higher_power"; }
  else if (/同事|客户|甲方|乙方|colleague|client/u.test(value)) { domain = "work"; role = /客户|甲方|client/u.test(value) ? "client" : "colleague"; }
  else if (/妈妈|母亲|爸爸|父亲|父母|家人|亲戚|兄弟|姐妹|哥哥|姐姐|弟弟|妹妹/u.test(value)) { domain = "family"; role = /妈妈|母亲|爸爸|父亲|父母/u.test(value) ? "parent" : /兄弟|姐妹|哥哥|姐姐|弟弟|妹妹/u.test(value) ? "sibling" : "relative"; powerPosition = "unknown"; }
  else if (/朋友|闺蜜|兄弟|同学|friend/u.test(value)) { domain = "friendship"; role = "friend"; }
  const goal = /怎么回|回复|怎么说|消息|聊天记录/u.test(value) ? "draft_reply" : /拒绝/u.test(value) ? "refuse" : /边界|不想|别再/u.test(value) ? "set_boundary" : /道歉|和好|修复/u.test(value) ? "repair" : /判断|什么意思|怎么看/u.test(value) ? "interpret_signal" : "other";
  const stage = /刚认识|第一次|初识/u.test(value) ? "new" : /分手|断联/u.test(value) ? "separated" : /冷战/u.test(value) ? "cooling" : /冲突|吵架/u.test(value) ? "conflict" : /稳定|长期|多年/u.test(value) ? "stable" : "unknown";
  return { domain, role, stage, powerPosition, goal };
}

function relationshipPolicyContext(classification) {
  const role = classification?.role || classification?.relationshipRole || "other";
  if (role === "dating") return "暧昧或约会关系：保留轻盈和可退空间，不替对方读心；建议回复要短、自然、不过度承诺。";
  if (role === "partner") return "伴侣关系：优先说清需求和具体行为，不把一次冲突上升为人格判断；修复建议必须允许双方不同步。";
  if (role === "ex") return "前任关系：优先保护边界和复原，不以复合为默认目标；避免制造稀缺感、嫉妒或情绪操控。";
  if (role === "friend") return "朋友关系：兼顾真诚与体面，区分偶发疏忽和长期失衡；建议应保留关系，也允许降低投入。";
  if (["manager", "report", "client", "colleague"].includes(role)) return "职场关系：用事实、影响、请求组织表达；注意权力差异、书面留痕和职业风险，不鼓励冲动对抗。";
  if (["parent", "child", "sibling", "relative"].includes(role)) return "家庭关系：承认长期角色和现实牵连，但不以亲情要求无限忍让；把边界落到频率、话题或具体行为。";
  return "关系尚未明确：只基于已知信息给低风险建议；若缺失的关系角色会改变判断，只追问一个具体问题。";
}

function relationshipSafety(text) {
  const value = String(text || "");
  if (/掐|打我|家暴|强迫|威胁|跟踪|骚扰|偷拍视频|下药|控制我|不让我走/u.test(value)) return "coercion";
  if (/未成年|未满18|学生和老师|师生恋/u.test(value)) return "minor";
  if (/开除|降职|报复|穿小鞋|权色交易/u.test(value)) return "work_retaliation";
  return "none";
}

function redactRelationshipSummary(value, max = 800) {
  return [...String(value || "").replace(/(?:1[3-9]\d{9})/g, "[手机号]").replace(/[\w.+-]+@[\w.-]+\.[A-Za-z]{2,}/g, "[邮箱]").replace(/\s+/g, " ").trim()].slice(0, max).join("");
}
...
async function migrateLegacyRelationships(env, userId) {
  const account = await readAccountData(env.DB, userId);
  const people = Array.isArray(account.settings?.privatePeople) ? account.settings.privatePeople.slice(0, 40) : [];
  const loops = Array.isArray(account.settings?.relationshipLoops) ? account.settings.relationshipLoops.slice(0, 100) : [];
  const existing = await env.DB.prepare("SELECT id, legacy_person_id AS legacyPersonId FROM relationship_people WHERE owner_user_id = ?").bind(userId).all();
  const personIds = new Map((existing.results || []).map((item) => [String(item.legacyPersonId || ""), String(item.id)]));
  ...
  for (const loop of loops) {
    const legacyId = String(loop?.id || "").slice(0, 120);
    const personId = personIds.get(String(loop?.personId || ""));
    const summary = redactRelationshipSummary(loop?.situation, 800);
    if (!legacyId || !personId || !summary) continue;
    const duplicate = await env.DB.prepare("SELECT id FROM relationship_cases WHERE owner_user_id = ? AND id = ?").bind(userId, "legacy:" + legacyId).first();
    if (duplicate) continue;
    const classification = relationshipClassification(summary + " " + String(loop?.need || ""));
    const now = String(loop?.updatedAt || loop?.createdAt || new Date().toISOString()).slice(0, 40);
    await env.DB.prepare("INSERT INTO relationship_cases (id, owner_user_id, person_id, goal, status, source, initial_text_redacted, strategy_key, created_at, updated_at, resolved_at) VALUES (?, ?, ?, ?, ?, 'person_mirror', ?, ?, ?, ?, ?)")
      .bind("legacy:" + legacyId, userId, personId, classification.goal, loop?.status === "reported" ? "resolved" : "open", summary, classification.role, String(loop?.createdAt || now).slice(0, 40), now, loop?.reportedAt ? String(loop.reportedAt).slice(0, 40) : null).run();
  }
}
...
function relationshipCaseRow(row) {
  return { id: row.id, personId: row.personId || undefined, goal: row.goal, status: row.status, source: row.source, strategyKey: row.strategyKey, summary: row.summary || undefined, recommendedReply: row.recommendedReply || undefined, createdAt: row.createdAt, updatedAt: row.updatedAt, resolvedAt: row.resolvedAt || undefined };
}
...
async function sessionUser(request, env) {
  ...
  return env.DB.prepare("SELECT users.id, users.email, users.display_name AS displayName, users.provider FROM identity_sessions AS sessions JOIN identity_users AS users ON users.id = sessions.user_id WHERE sessions.token_hash = ? AND sessions.expires_at > ?")
...
async function getOrCreateUser(env, email, displayName, provider) {
  let user = await env.DB.prepare("SELECT id, email, display_name AS displayName, provider FROM identity_users WHERE email = ?").bind(email).first();
  if (user) {
  ...
  return { id, email, displayName: displayName || null, provider, created: true };
}
...
async function resolveBetaInvite(env, rawCode) {
  ...
  const codeHash = await sha256("lifemirror-beta|" + code);
  let invite = await env.DB.prepare("SELECT id, label, batch, max_uses AS maxUses, use_count AS useCount, expires_at AS expiresAt, revoked_at AS revokedAt FROM beta_invites WHERE code_hash = ?").bind(codeHash).first();
  if (!invite) {
    const configured = String(env.BETA_INVITE_CODES || "").split(",").map((item) => normalizeInviteCode(item)).filter(Boolean);
    if (!configured.includes(code)) return { error: "invalid_invite" };
    const now = new Date(); const id = crypto.randomUUID();
    const maxUses = Math.min(500, Math.max(1, Number(env.BETA_INVITE_MAX_USES || 50)));
    const expiryDays = Math.min(180, Math.max(1, Number(env.BETA_INVITE_EXPIRY_DAYS || 45)));
    const expiresAt = new Date(now.getTime() + expiryDays * 86400000).toISOString();
    await env.DB.prepare("INSERT OR IGNORE INTO beta_invites (id, code_hash, label, batch, max_uses, use_count, expires_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 0, ?, ?, ?)")
      .bind(id, codeHash, "首轮体验测试", String(env.BETA_INVITE_BATCH || "beta-01").slice(0, 40), maxUses, expiresAt, now.toISOString(), now.toISOString()).run();
    invite = await env.DB.prepare("SELECT id, label, batch, max_uses AS maxUses, use_count AS useCount, expires_at AS expiresAt, revoked_at AS revokedAt FROM beta_invites WHERE code_hash = ?").bind(codeHash).first();
  }
  if (!invite) return { error: "invalid_invite" };
  if (invite.revokedAt) return { error: "invite_revoked" };
  if (invite.expiresAt && Date.parse(invite.expiresAt) <= Date.now()) return { error: "invite_expired" };
  if (Number(invite.useCount || 0) >= Number(invite.maxUses || 0)) return { error: "invite_full" };
  return { invite };
}
...
async function acceptBetaInvite(request, env) {
  ...
  const invite = resolved.invite; const userId = crypto.randomUUID(); const now = new Date().toISOString();
  const syntheticEmail = "invite-" + userId + "@anonymous.lifemirror.invalid";
  await env.DB.batch([
    env.DB.prepare("UPDATE beta_invites SET use_count = use_count + 1, updated_at = ? WHERE id = ? AND revoked_at IS NULL AND use_count < max_uses").bind(now, invite.id),
    env.DB.prepare("INSERT INTO identity_users (id, email, display_name, provider, created_at, updated_at) VALUES (?, ?, NULL, 'invite', ?, ?)").bind(userId, syntheticEmail, now, now),
    env.DB.prepare("INSERT INTO beta_participants (id, user_id, invite_id, batch, created_at) VALUES (?, ?, ?, ?, ?)").bind(crypto.randomUUID(), userId, invite.id, invite.batch, now),
  ]);
  const data = await migrateGuestData(env, userId, input);
  await env.DB.prepare("INSERT OR IGNORE INTO product_metric_events (id, user_id, event_type, surface, event_key, occurred_at) VALUES (?, ?, 'invite_accepted', 'account', ?, ?)")
    .bind(crypto.randomUUID(), userId, "invite:" + invite.id, now).run();
  const token = await createSession(env, userId);
  return json({ authenticated: true, created: true, user: { id: userId, email: null, displayName: null, provider: "invite", batch: invite.batch }, data }, 201, { "set-cookie": sessionCookie(token) });
}
...
async function verifyCode(request, env) {
  ...
  if (["invite", "referral"].includes(invitedUser?.provider)) {
    ...
    await env.DB.prepare("INSERT OR IGNORE INTO product_metric_events (id, user_id, event_type, surface, event_key, occurred_at) VALUES (?, ?, 'account_bound', 'account', ?, ?)")
      .bind(crypto.randomUUID(), user.id, "bound:" + user.id, new Date().toISOString()).run();
  } else user = await getOrCreateUser(env, email, null, "email");
  const data = await migrateGuestData(env, user.id, input);
  const token = await createSession(env, user.id);
  return json({ authenticated: true, created: user.created, user: { id: user.id, email: user.email, displayName: user.displayName, provider: user.provider || "email" }, data }, 200, { "set-cookie": sessionCookie(token) });
}
...
async function authApi(request, env, pathname) {
  ...
  if (pathname === "/api/v1/account/relationships" && request.method === "GET") {
  ...
  if (pathname === "/api/v1/account/relationships/people" && request.method === "POST") {
    if (String(env.RELATIONSHIP_NORMALIZED_WRITE_ENABLED || "true") === "false") return json({ error: "relationship_normalized_write_disabled" }, 503);
    const input = await body(request); const displayName = redactRelationshipSummary(input?.displayName, 40);
    const relationshipLabel = redactRelationshipSummary(input?.relationshipLabel, 40); const legacyPersonId = String(input?.legacyPersonId || "").slice(0, 120) || null;
    if (!displayName) return json({ error: "invalid_relationship_person" }, 400);
    const classification = relationshipClassification(relationshipLabel + " " + displayName); const now = new Date().toISOString();
    const prior = legacyPersonId ? await env.DB.prepare("SELECT id, created_at AS createdAt FROM relationship_people WHERE owner_user_id = ? AND legacy_person_id = ?").bind(user.id, legacyPersonId).first() : null;
  ...
  if (pathname === "/api/v1/account/relationships/cases" && request.method === "POST") {
    if (String(env.RELATIONSHIP_NORMALIZED_WRITE_ENABLED || "true") === "false") return json({ error: "relationship_normalized_write_disabled" }, 503);
    const input = await body(request); const text = redactRelationshipSummary(input?.text, 800); const source = input?.source === "screenshot" ? "screenshot" : "text";
    const recommendedReply = redactRelationshipSummary(input?.recommendedReply, 500) || null; const personId = String(input?.personId || "").slice(0, 120) || null;
    if (text.length < 2) return json({ error: "invalid_relationship_case" }, 400);
    if (personId && !(await env.DB.prepare("SELECT id FROM relationship_people WHERE id = ? AND owner_user_id = ? AND archived_at IS NULL").bind(personId, user.id).first())) return json({ error: "relationship_person_not_found" }, 404);
    const classification = relationshipClassification(text); const id = crypto.randomUUID(), now = new Date().toISOString();
    await env.DB.prepare("INSERT INTO relationship_cases (id, owner_user_id, person_id, goal, status, source, initial_text_redacted, strategy_key, recommended_reply, created_at, updated_at) VALUES (?, ?, ?, ?, 'open', ?, ?, ?, ?, ?, ?)")
      .bind(id, user.id, personId, classification.goal, source, text, classification.role, recommendedReply, now, now).run();
    const userNote = redactRelationshipSummary(input?.userNote || input?.text, 800);
    await env.DB.prepare("INSERT INTO relationship_events (id, owner_user_id, person_id, case_id, event_kind, payload_json, provenance, created_at) VALUES (?, ?, ?, ?, 'user_message', ?, 'user_authored', ?)")
      .bind(crypto.randomUUID(), user.id, personId, id, JSON.stringify({ version: 2, summary: userNote, source }), now).run();
    const extracted = Array.isArray(input?.extractedConversation) ? input.extractedConversation.slice(0, 80) : [];
    for (const [order, raw] of extracted.entries()) {
      const speaker = ["user", "other", "unknown"].includes(raw?.speaker) ? raw.speaker : "unknown";
      const messageText = redactRelationshipSummary(raw?.text, 1000); if (!messageText) continue;
      const payload = { version: 1, order, speaker, text: messageText, ...(raw?.visibleTime ? { visibleTime: redactRelationshipSummary(raw.visibleTime, 80) } : {}), ...(Array.isArray(raw?.signals) ? { signals: raw.signals.slice(0, 8).map((item) => redactRelationshipSummary(item, 80)) } : {}), ...(raw?.uncertain === true ? { uncertain: true } : {}), ...(Number.isInteger(raw?.pageOrder) ? { pageOrder: raw.pageOrder } : {}), ...(Number.isInteger(raw?.messageOrder) ? { messageOrder: raw.messageOrder } : {}) };
      await env.DB.prepare("INSERT INTO relationship_events (id, owner_user_id, person_id, case_id, event_kind, payload_json, provenance, created_at) VALUES (?, ?, ?, ?, 'extracted_message', ?, 'model_extracted', ?)").bind(crypto.randomUUID(), user.id, personId, id, JSON.stringify(payload), now).run();
    }
    const analysisSummary = redactRelationshipSummary(input?.analysisSummary, 4000);
    if (analysisSummary) {
      const replyOptions = Array.isArray(input?.replyOptions) ? input.replyOptions.slice(0, 4).map((item) => ({ id: redactRelationshipSummary(item?.id, 40), tone: ["natural", "warm", "direct", "boundary"].includes(item?.tone) ? item.tone : "natural", text: redactRelationshipSummary(item?.text, 180), why: redactRelationshipSummary(item?.why, 300) })).filter((item) => item.text) : [];
      await env.DB.prepare("INSERT INTO relationship_events (id, owner_user_id, person_id, case_id, event_kind, payload_json, provenance, created_at) VALUES (?, ?, ?, ?, 'analysis_created', ?, 'model_generated', ?)").bind(crypto.randomUUID(), user.id, personId, id, JSON.stringify({ version: 1, summary: analysisSummary, replyOptions }), now).run();
    }
    const item = await env.DB.prepare("SELECT id, person_id AS personId, goal, status, source, strategy_key AS strategyKey, initial_text_redacted AS summary, recommended_reply AS recommendedReply, created_at AS createdAt, updated_at AS updatedAt, resolved_at AS resolvedAt FROM relationship_cases WHERE id = ? AND owner_user_id = ?").bind(id, user.id).first();
    return json({ case: relationshipCaseRow(item) }, 201);
  }
  if (pathname === "/api/v1/account/relationships/feedback" && request.method === "POST") {
    if (String(env.RELATIONSHIP_NORMALIZED_WRITE_ENABLED || "true") === "false") return json({ error: "relationship_normalized_write_disabled" }, 503);
    const input = await body(request); const caseId = String(input?.caseId || "").slice(0, 120); const outcomes = ["positive", "mixed", "negative", "no_response", "not_yet"];
    const outcome = outcomes.includes(input?.outcome) ? input.outcome : "not_yet"; const item = caseId ? await env.DB.prepare("SELECT id, person_id AS personId FROM relationship_cases WHERE id = ? AND owner_user_id = ?").bind(caseId, user.id).first() : null;
    if (!item) return json({ error: "relationship_case_not_found" }, 404);
    const now = new Date().toISOString(), feedbackId = crypto.randomUUID(); const actualReply = redactRelationshipSummary(input?.actualReply, 500) || null;
    await env.DB.prepare("INSERT INTO relationship_feedback (id, owner_user_id, person_id, case_id, acted, outcome, actual_reply_summary, suggestion_helpful, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")
      .bind(feedbackId, user.id, item.personId || null, caseId, input?.acted === true ? 1 : 0, outcome, actualReply, typeof input?.suggestionHelpful === "boolean" ? Number(input.suggestionHelpful) : null, now).run();
    await env.DB.prepare("INSERT INTO relationship_events (id, owner_user_id, person_id, case_id, event_kind, payload_json, provenance, created_at) VALUES (?, ?, ?, ?, 'reality_feedback', ?, 'user_authored', ?)")
      .bind(crypto.randomUUID(), user.id, item.personId || null, caseId, JSON.stringify({ version: 1, acted: input?.acted === true, outcome, note: redactRelationshipSummary(input?.note, 300) || undefined }), now).run();
    await env.DB.prepare("UPDATE relationship_cases SET status = ?, resolved_at = ?, updated_at = ? WHERE id = ? AND owner_user_id = ?").bind(outcome === "not_yet" ? "awaiting_reply" : "resolved", outcome === "not_yet" ? null : now, now, caseId, user.id).run();
    return json({ feedback: { id: feedbackId, caseId, acted: input?.acted === true, outcome, createdAt: now } }, 201);
  }
  const relationshipCaseMatch = pathname.match(/^\/api\/v1\/account\/relationships\/cases\/([^/]+)$/);
  if (relationshipCaseMatch && request.method === "PATCH") {
    if (String(env.RELATIONSHIP_NORMALIZED_WRITE_ENABLED || "true") === "false") return json({ error: "relationship_normalized_write_disabled" }, 503);
    const input = await body(request); const caseId = decodeURIComponent(relationshipCaseMatch[1]); const personId = String(input?.personId || "").slice(0, 120);
    const person = personId ? await env.DB.prepare("SELECT id FROM relationship_people WHERE id = ? AND owner_user_id = ? AND archived_at IS NULL").bind(personId, user.id).first() : null;
    if (!person) return json({ error: "relationship_person_not_found" }, 404);
    const now = new Date().toISOString(); const result = await env.DB.prepare("UPDATE relationship_cases SET person_id = ?, updated_at = ? WHERE id = ? AND owner_user_id = ? AND status <> 'archived'").bind(personId, now, caseId, user.id).run();
    if (!result.meta?.changes) return json({ error: "relationship_case_not_found" }, 404);
    await env.DB.prepare("UPDATE relationship_events SET person_id = ? WHERE case_id = ? AND owner_user_id = ?").bind(personId, caseId, user.id).run();
    await env.DB.prepare("UPDATE relationship_feedback SET person_id = ? WHERE case_id = ? AND owner_user_id = ?").bind(personId, caseId, user.id).run();
    await env.DB.prepare("INSERT INTO relationship_events (id, owner_user_id, person_id, case_id, event_kind, payload_json, provenance, created_at) VALUES (?, ?, ?, ?, 'classification_confirmed', ?, 'user_authored', ?)").bind(crypto.randomUUID(), user.id, personId, caseId, JSON.stringify({ version: 1, linkedAfterAnswer: true }), now).run();
    const item = await env.DB.prepare("SELECT id, person_id AS personId, goal, status, source, strategy_key AS strategyKey, initial_text_redacted AS summary, recommended_reply AS recommendedReply, created_at AS createdAt, updated_at AS updatedAt, resolved_at AS resolvedAt FROM relationship_cases WHERE id = ? AND owner_user_id = ?").bind(caseId, user.id).first();
    return json({ case: relationshipCaseRow(item) });
  }
  if (relationshipCaseMatch && request.method === "DELETE") {
    const caseId = decodeURIComponent(relationshipCaseMatch[1]); const now = new Date().toISOString();
    const result = await env.DB.prepare("UPDATE relationship_cases SET status = 'archived', updated_at = ? WHERE id = ? AND owner_user_id = ?").bind(now, caseId, user.id).run();
    return result.meta?.changes ? new Response(null, { status: 204 }) : json({ error: "relationship_case_not_found" }, 404);
  }
  ...
  if (relationshipPersonContextMatch && request.method === "GET") {
    ...
    const [cases, events, feedback] = await Promise.all([
      ...
      env.DB.prepare("SELECT case_id AS caseId, outcome, acted, actual_reply_summary AS note, created_at AS createdAt FROM relationship_feedback WHERE owner_user_id = ? AND person_id = ? ORDER BY created_at DESC LIMIT 5").bind(user.id, personId).all(),
    ]);
    const parsedEvents = (events.results || []).map((item) => { try { return { ...item, payload: JSON.parse(String(item.payloadJson || "{}")) }; } catch { return { ...item, payload: {} }; } });
    const extractedMessages = parsedEvents.filter((item) => item.eventKind === "extracted_message").slice(0, 30).reverse().map((item) => ({ caseId: item.caseId, createdAt: item.createdAt, speaker: item.payload.speaker, text: item.payload.text, visibleTime: item.payload.visibleTime, signals: item.payload.signals, uncertain: item.payload.uncertain }));
    const priorAnalyses = parsedEvents.filter((item) => item.eventKind === "analysis_created").slice(0, 3).map((item) => ({ caseId: item.caseId, createdAt: item.createdAt, summary: item.payload.summary }));
    return json({ context: { recentCases: (cases.results || []).map(relationshipCaseRow), extractedMessages, priorAnalyses, realityFeedback: (feedback.results || []).map((item) => ({ ...item, acted: Boolean(item.acted) })) } });
  }
  const relationshipPersonMatch = pathname.match(/^\/api\/v1\/account\/relationships\/people\/([^/]+)$/);
  if (relationshipPersonMatch && request.method === "DELETE") {
    const personId = decodeURIComponent(relationshipPersonMatch[1]); const now = new Date().toISOString();
    const result = await env.DB.prepare("UPDATE relationship_people SET archived_at = ?, updated_at = ? WHERE id = ? AND owner_user_id = ?").bind(now, now, personId, user.id).run();
    return result.meta?.changes ? new Response(null, { status: 204 }) : json({ error: "relationship_person_not_found" }, 404);
  }
  ...
  if (pathname === "/api/v1/account/product-metrics/events" && request.method === "POST") {
    ...
    const eventTypes = ["chat_message_sent","daily_opened","daily_checkin_completed","mirror_result_ready","tool_continued_chat","share_card_shared","share_link_created","share_response_created","first_reply_received","conversation_continued","conversation_mode_changed","life_loop_created","life_loop_feedback","memory_recall_positive","memory_recall_negative","share_intent","invite_accepted","onboarding_started","onboarding_prompt_used","generation_stopped","generation_retried","chat_feedback_helpful","chat_feedback_missed","account_bound","relationship_entry_opened","relationship_text_submitted","relationship_image_submitted","relationship_clarification_shown","relationship_clarification_answered","relationship_answer_received","relationship_reply_copied","relationship_person_saved","relationship_case_revisited","relationship_feedback_submitted","relationship_outcome_positive","relationship_outcome_negative","vision_parse_failed"];
...
async function runReleaseAcceptance(env) {
  ...
  const checks = {
    ...
    normalizedRelationshipIsolation: false,
  ...
  try {
    ...
    const privatePersonA = crypto.randomUUID(), privatePersonB = crypto.randomUUID(), privateCaseA = crypto.randomUUID();
    await env.DB.batch([
      env.DB.prepare("INSERT INTO relationship_people (id, owner_user_id, display_name, relationship_label, domain, role, stage, power_position, classification_source, confirmed_by_user, created_at, updated_at) VALUES (?, ?, 'A private person', '朋友', 'friendship', 'friend', 'unknown', 'roughly_equal', 'user_confirmed', 1, ?, ?)").bind(privatePersonA, userA, now, now),
      env.DB.prepare("INSERT INTO relationship_people (id, owner_user_id, display_name, relationship_label, domain, role, stage, power_position, classification_source, confirmed_by_user, created_at, updated_at) VALUES (?, ?, 'B private person', '同事', 'work', 'colleague', 'unknown', 'roughly_equal', 'user_confirmed', 1, ?, ?)").bind(privatePersonB, userB, now, now),
    ]);
    await env.DB.prepare("INSERT INTO relationship_cases (id, owner_user_id, person_id, goal, status, source, initial_text_redacted, strategy_key, created_at, updated_at) VALUES (?, ?, ?, 'interpret_signal', 'open', 'text', 'synthetic', 'friendship', ?, ?)").bind(privateCaseA, userA, privatePersonA, now, now).run();
    const aVisible = await env.DB.prepare("SELECT count(*) AS total FROM relationship_cases WHERE owner_user_id = ? AND person_id = ?").bind(userA, privatePersonA).first();
    const bCannotSee = await env.DB.prepare("SELECT count(*) AS total FROM relationship_cases WHERE owner_user_id = ? AND id = ?").bind(userB, privateCaseA).first();
    checks.normalizedRelationshipIsolation = Number(aVisible?.total || 0) === 1 && Number(bCannotSee?.total || 0) === 0;
  } catch {
  ...
  await env.DB.prepare("INSERT INTO release_acceptance_runs (id, source_commit, schema_version, result, checks_json, created_at) VALUES (?, ?, 16, ?, ?, ?)")
    .bind(runId, String(env.SOURCE_COMMIT || "unknown"), passed ? "passed" : "failed", JSON.stringify(checks), new Date().toISOString()).run();
  return { passed, checks, sourceCommit: String(env.SOURCE_COMMIT || "unknown"), schemaVersion: 16, runId };
}

async function opsApi(request, env, pathname) {
  if (pathname === "/api/v1/ops/health" && request.method === "GET") return json({ status: "ok", phase: "promotion-readiness", sourceCommit: String(env.SOURCE_COMMIT || "unknown"), schemaVersion: 16, runtimeVersions: RUNTIME_VERSIONS });
  ...
  if (pathname === "/api/v1/ops/invites" && request.method === "GET") {
    const rows = await env.DB.prepare("SELECT invites.id, invites.label, invites.batch, invites.max_uses AS maxUses, invites.use_count AS useCount, invites.expires_at AS expiresAt, invites.revoked_at AS revokedAt, invites.created_at AS createdAt, count(participants.id) AS participants, sum(CASE WHEN participants.bound_email_at IS NOT NULL THEN 1 ELSE 0 END) AS boundAccounts FROM beta_invites AS invites LEFT JOIN beta_participants AS participants ON participants.invite_id = invites.id GROUP BY invites.id ORDER BY invites.created_at DESC LIMIT 50").all();
    return json({ invites: rows.results || [], privacy: "codes_are_never_returned" });
  }
  if (pathname === "/api/v1/ops/invites" && request.method === "POST") {
    const input = await body(request); const code = normalizeInviteCode(input?.code);
    const maxUses = Math.min(500, Math.max(1, Number(input?.maxUses || 10)));
    const label = String(input?.label || "体验测试").replace(/\s+/g, " ").trim().slice(0, 60);
    const batch = String(input?.batch || "beta-01").replace(/[^a-zA-Z0-9_-]/g, "").slice(0, 40);
    const expiresAt = input?.expiresAt && !Number.isNaN(Date.parse(input.expiresAt)) ? new Date(input.expiresAt).toISOString() : new Date(Date.now() + 45 * 86400000).toISOString();
    if (!code || !label || !batch) return json({ error: "invalid_beta_invite" }, 400);
    const id = crypto.randomUUID(); const now = new Date().toISOString();
    try {
      await env.DB.prepare("INSERT INTO beta_invites (id, code_hash, label, batch, max_uses, use_count, expires_at, created_at, updated_at) VALUES (?, ?, ?, ?, ?, 0, ?, ?, ?)")
        .bind(id, await sha256("lifemirror-beta|" + code), label, batch, maxUses, expiresAt, now, now).run();
    } catch { return json({ error: "invite_code_exists" }, 409); }
    return json({ invite: { id, code, label, batch, maxUses, expiresAt }, warning: "code_is_shown_once" }, 201);
  }
  if (pathname === "/api/v1/ops/invites" && request.method === "PATCH") {
    const input = await body(request); const id = String(input?.inviteId || "").slice(0, 120); const revoked = input?.revoked !== false; const now = new Date().toISOString();
    if (!id) return json({ error: "invalid_beta_invite" }, 400);
    const result = await env.DB.prepare("UPDATE beta_invites SET revoked_at = ?, updated_at = ? WHERE id = ?").bind(revoked ? now : null, now, id).run();
    return Number(result.meta?.changes || 0) ? json({ ok: true, revoked }) : json({ error: "invite_not_found" }, 404);
  }
...
function bytesToBase64(buffer) {
  ...
  for (let offset = 0; offset < bytes.length; offset += 0x8000) output += String.fromCharCode(...bytes.subarray(offset, Math.min(offset + 0x8000, bytes.length)));
  return btoa(output);
}

function speakerFromBubbleSide(side) {
  return side === "right" ? "user" : side === "left" ? "other" : "unknown";
}

async function visionExtract(request, env) {
  if (request.method !== "POST") return new Response("Method not allowed", { status: 405 });
  if (String(env.VISION_INPUT_ENABLED || "true") === "false") return json({ error: "vision_disabled" }, 503);
  const form = await request.formData().catch(() => null); if (!form) return json({ error: "invalid_vision_request" }, 400);
  const images = form.getAll("images").filter((item) => item && typeof item.arrayBuffer === "function");
  const allowed = new Set(["image/jpeg", "image/png", "image/webp"]);
  if (!images.length || images.length > 3 || images.some((image) => !allowed.has(image.type) || image.size > 1_200_000) || images.reduce((sum, image) => sum + image.size, 0) > 3_200_000) return json({ error: "invalid_image" }, 400);
  const apiKey = env.VISION_API_KEY || env.LLM_FALLBACK_API_KEY; const baseUrl = String(env.VISION_BASE_URL || env.LLM_FALLBACK_BASE_URL || "https://api.groq.com/openai/v1").replace(/\/$/, "");
  const model = String(env.VISION_MODEL || "qwen/qwen3.6-27b"); if (!apiKey) return json({ error: "vision_not_configured" }, 503);
  const buffers = await Promise.all(images.map((image) => image.arrayBuffer()));
  const content = [{ type: "text", text: "按图片顺序提取聊天截图中肉眼可见的对话。先只判断最外层聊天气泡的版面位置，不根据称呼、语气或文字含义猜说话人。位置规则固定：靠左气泡写 side='left'，靠右气泡写 side='right'，居中的时间、系统提示或通知写 side='center'，无法确定写 side='unknown'。每个最外层气泡只生成一条 message；引用、回复预览或转发卡片属于所在外层气泡，不要拆成另一位说话人的新消息。保留可见时间以及已读、撤回、转账、表情等画面信号；看不清的文字不要猜。不要输出 speaker 或推断人物动机。只返回 JSON：{pages:[{order,messages:[{side:'left|right|center|unknown',text,visibleTime?,signals?,uncertain?}]}],missingRegions:[],warnings:[]}。" }, ...buffers.map((buffer, index) => ({ type: "image_url", image_url: { url: "data:" + images[index].type + ";base64," + bytesToBase64(buffer) } }))];
  try {
    const upstream = await fetch(baseUrl + "/chat/completions", { method: "POST", headers: { authorization: "Bearer " + apiKey, "content-type": "application/json" }, body: JSON.stringify({ model, temperature: 0, max_tokens: 2400, reasoning_effort: "none", response_format: { type: "json_object" }, messages: [{ role: "user", content }] }), signal: AbortSignal.timeout(45_000) });
    if (upstream.status === 429) return json({ error: "vision_rate_limited" }, 429, { "cache-control": "no-store" });
    if (!upstream.ok) return json({ error: "vision_upstream_failed" }, 502, { "cache-control": "no-store" });
    const payload = await upstream.json(); const parsed = extractJson(payload?.choices?.[0]?.message?.content || "");
    if (!Array.isArray(parsed?.pages) || parsed.pages.length > 3) throw new Error("vision_invalid_payload");
    const user = await sessionUser(request, env); const now = new Date().toISOString(); const ids = images.map(() => crypto.randomUUID());
    if (user) for (let index = 0; index < images.length; index += 1) await env.DB.prepare("INSERT INTO attachment_metadata (id, owner_user_id, mime_type, byte_size, content_hash, parse_status, created_at, deleted_at) VALUES (?, ?, ?, ?, ?, 'parsed', ?, ?)").bind(ids[index], user.id, images[index].type, images[index].size, await sha256Bytes(buffers[index]), now, now).run();
    const bubbleSides = parsed.pages.flatMap((page) => Array.isArray(page?.messages) ? page.messages.map((message) => message?.side) : []).filter((side) => ["left", "right", "center", "unknown"].includes(side));
    const pages = parsed.pages.map((page, index) => ({ attachmentId: ids[index] || crypto.randomUUID(), order: index, messages: Array.isArray(page?.messages) ? page.messages.slice(0, 80).map((message) => ({ speaker: speakerFromBubbleSide(message?.side), text: redactRelationshipSummary(message?.text, 2000), ...(message?.visibleTime ? { visibleTime: redactRelationshipSummary(message.visibleTime, 80) } : {}), ...(Array.isArray(message?.signals) ? { signals: message.signals.slice(0, 8).map((item) => redactRelationshipSummary(item, 80)) } : {}), ...(message?.uncertain === true ? { uncertain: true } : {}) })).filter((message) => message.text) : [] }));
    return json({ conversation: { pages, inferredUserSide: bubbleSides.some((side) => side === "left" || side === "right") ? "right" : "unknown", missingRegions: Array.isArray(parsed.missingRegions) ? parsed.missingRegions.slice(0, 10) : [], warnings: Array.isArray(parsed.warnings) ? parsed.warnings.slice(0, 10) : [] }, attachmentIds: ids }, 200, { "cache-control": "no-store" });
  } catch { return json({ error: "vision_parse_failed" }, 502); }
}
...
async function nextModelDelta(state) {
  while (true) {
    const newline = state.buffer.indexOf("\n");
    if (newline >= 0) {
      const line = state.buffer.slice(0, newline).trim(); state.buffer = state.buffer.slice(newline + 1);
      if (!line.startsWith("data:")) continue;
      const data = line.slice(5).trim();
      if (!data) continue;
      if (data === "[DONE]") return { done: true, text: "" };
      try { const text = modelText(JSON.parse(data)); if (text) return { done: false, text }; } catch {}
      continue;
    }
    const chunk = await state.reader.read();
    if (chunk.done) {
      const tail = state.buffer.trim(); state.buffer = "";
      if (tail.startsWith("data:")) {
        const data = tail.slice(5).trim();
        if (data && data !== "[DONE]") { try { const text = modelText(JSON.parse(data)); if (text) return { done: false, text }; } catch {} }
      }
...
function browserSse(type, text = "") {
  return new TextEncoder().encode("data: " + JSON.stringify({ type, text }) + "\n\n");
}

async function openChatStream(env, payload, { timeoutMs = 45_000, userId = null } = {}) {
  const providers = modelProviders(env);
  if (!providers[0].apiKey || !providers[0].model) throw new Error("llm_not_configured");
  const budget = await checkDailyLlmBudget(env);
  if (!budget.allowed) throw new Error("llm_daily_budget_exceeded");
  const inputBytes = new TextEncoder().encode(JSON.stringify(payload)).byteLength;
  let lastError = new Error("llm_unavailable");
  for (const [index, provider] of providers.entries()) {
    ...
    try {
      const upstream = await fetch(provider.baseUrl + "/chat/completions", {
        ...
        body: JSON.stringify({ ...payload, model: provider.model, stream: true }),
        signal: AbortSignal.timeout(timeoutMs),
      });
      if (!upstream.ok) {
        const failure = new Error("llm_upstream_" + upstream.status); failure.retryable = shouldRetryProvider(upstream.status); throw failure;
      }
      const contentType = upstream.headers.get("content-type") || "";
      if (!contentType.includes("text/event-stream")) {
        const text = decodeModelResponse(await upstream.text());
        if (!text.trim()) throw new Error("llm_empty_response");
        const stream = new ReadableStream({ start(controller) { controller.enqueue(browserSse("delta", text)); controller.enqueue(browserSse("done")); controller.close(); } });
        await auditLlmCall(env, { userId, operation: "chat", provider: provider.name, model: provider.model, outcome: "succeeded", latencyMs: Date.now() - started, inputBytes, outputBytes: new TextEncoder().encode(text).byteLength, cost: estimatedLlmCost(env, inputBytes, new TextEncoder().encode(text).byteLength) }).catch(() => undefined);
        return { stream, provider: provider.name };
      }
      const state = { reader: upstream.body.getReader(), decoder: new TextDecoder(), buffer: "" };
      const first = await nextModelDelta(state);
      if (first.done || !first.text) throw new Error("llm_empty_response");
      let outputBytes = new TextEncoder().encode(first.text).byteLength;
      const stream = new ReadableStream({
        start(controller) {
          controller.enqueue(browserSse("delta", first.text));
          (async () => {
            try {
              while (true) {
                const next = await nextModelDelta(state);
                if (next.done) break;
                if (next.text) { outputBytes += new TextEncoder().encode(next.text).byteLength; controller.enqueue(browserSse("delta", next.text)); }
              }
              controller.enqueue(browserSse("done")); controller.close();
              await auditLlmCall(env, { userId, operation: "chat", provider: provider.name, model: provider.model, outcome: "succeeded", latencyMs: Date.now() - started, inputBytes, outputBytes, cost: estimatedLlmCost(env, inputBytes, outputBytes) }).catch(() => undefined);
            } catch {
              controller.enqueue(browserSse("interrupted")); controller.close();
              await auditLlmCall(env, { userId, operation: "chat", provider: provider.name, model: provider.model, outcome: "interrupted", latencyMs: Date.now() - started, inputBytes, outputBytes, cost: estimatedLlmCost(env, inputBytes, outputBytes) }).catch(() => undefined);
            }
          })();
        },
        cancel() { state.reader.cancel().catch(() => undefined); },
      });
      return { stream, provider: provider.name };
    } catch (error) {
      lastError = error instanceof Error ? error : new Error("llm_request_failed");
      await auditLlmCall(env, { userId, operation: "chat", provider: provider.name, model: provider.model, outcome: lastError.name === "TimeoutError" || lastError.name === "AbortError" ? "timed_out" : "failed", latencyMs: Date.now() - started, inputBytes, outputBytes: 0, cost: estimatedLlmCost(env, inputBytes, 0) }).catch(() => undefined);
      if (index < providers.length - 1 && lastError.retryable !== false) continue;
    }
  }
  throw lastError;
...
async function shiguang(request, env) {
  ...
  const input = await body(request);
  if (input?.mode === "relationship" && String(env.RELATIONSHIP_ENGINE_ENABLED || "true") === "false") input.mode = "chat";
  if (!input || !["east", "west"].includes(input.theme) || ![undefined, "chat", "relationship", "daily_guidance", "mirror_result"].includes(input.mode) || (input.mode === "mirror_result" && !["tarot", "bazi", "astrology"].includes(input.kind)) || typeof input.context !== "string" || input.context.length > 12000 || !Array.isArray(input.messages) || input.messages.length < 1 || input.messages.length > 24) return json({ error: "invalid_chat_input" }, 400);
  ...
  const boundary = input.mode === "chat" || input.mode === "relationship" || !input.mode ? safetyBoundary(question) : "none";
  ...
  const requestedRelationship = input.relationship && typeof input.relationship === "object" ? input.relationship : null;
  const inferredRelationship = relationshipClassification(question);
  const classification = requestedRelationship && ["romance", "friendship", "work", "family", "other"].includes(requestedRelationship.domain) && ["dating", "partner", "ex", "friend", "colleague", "manager", "report", "client", "parent", "child", "sibling", "relative", "other"].includes(requestedRelationship.role) ? { ...inferredRelationship, ...requestedRelationship } : inferredRelationship;
  let normalizedHistory = "本轮没有已保存的关系案例。";
  if (user && input.mode === "relationship" && requestedRelationship?.personId) {
    const personId = String(requestedRelationship.personId).slice(0, 120);
    const ownsPerson = await env.DB.prepare("SELECT id FROM relationship_people WHERE id = ? AND owner_user_id = ? AND archived_at IS NULL").bind(personId, user.id).first();
    if (ownsPerson) {
      const history = await env.DB.prepare("SELECT cases.initial_text_redacted AS summary, cases.recommended_reply AS recommendedReply, cases.status, cases.updated_at AS updatedAt, (SELECT feedback.outcome FROM relationship_feedback AS feedback WHERE feedback.case_id = cases.id ORDER BY feedback.created_at DESC LIMIT 1) AS outcome, (SELECT feedback.acted FROM relationship_feedback AS feedback WHERE feedback.case_id = cases.id ORDER BY feedback.created_at DESC LIMIT 1) AS acted FROM relationship_cases AS cases WHERE cases.owner_user_id = ? AND cases.person_id = ? AND cases.status <> 'archived' ORDER BY cases.updated_at DESC LIMIT 3").bind(user.id, personId).all();
      normalizedHistory = (history.results || []).length ? (history.results || []).map((item) => "- [" + String(item.status) + "] " + String(item.summary || "") + (item.recommendedReply ? "；上次建议：" + String(item.recommendedReply) : "") + (item.outcome ? "；现实反馈：" + String(item.outcome) + "（" + (item.acted ? "已采用建议" : "未采用建议") + "）" : "")).join("\n") : normalizedHistory;
    }
  }
  ...
  const relationshipRisk = relationshipSafety(question);
  const relationshipSystem = governedChatSystem + "\n\n这一轮是持续可回访的现实关系工作台，不是一次性的新用户入口。这一步只做关系分析，不生成‘可以直接发’的话。先基于真正出现的文字和行为判断，不替 TA 读心，不把一次表现升级为人格结论。\n\n关系策略：" + relationshipPolicyContext(classification) + "\n\n回答自然包含三部分：先给现在更可能发生了什么的明确判断；再说清最容易误判、尚不能确认的一点；最后只给一个接下来要观察的现实信号。严格区分输入标签：[用户补充／主观感受] 是用户自己的经历与判断；[截图可见事实] 才是本轮画面证据。不要把用户补充伪装成截图事实，不能仅凭当前截图声称‘以前怎样’。不要虚构自己的恋爱或聊天经历，不说‘我以前也有过类似体验’。不得输出‘可以直接发、建议回复、你可以回’等回复区块；需要回复时由另一个受校验步骤完成。只有关系角色缺失会显著改变建议时才问一个短问题。不要输出内部分类、置信度或策略名。" + (relationshipRisk !== "none" ? "\n\n检测到现实安全风险（" + relationshipRisk + "）。停止优化操控或冒险话术，优先建议留存证据、联系可信的人和适当的现实专业支持；若有即时危险，优先离开现场并联系当地紧急服务。" : "") + "\n\n<normalized_relationship_history>\n" + normalizedHistory + "\n</normalized_relationship_history>";
  ...
  try {
    const system = input.mode === "mirror_result" ? mirrorResultSystem(input) : input.mode === "daily_guidance" ? dailyGuidanceSystem : input.mode === "relationship" ? relationshipSystem : governedChatSystem;
    if (input.mode === "chat" || input.mode === "relationship" || !input.mode) {
      const generated = await openChatStream(env, { temperature: 0.65, max_tokens: 900, messages: [{ role: "system", content: system }, ...messages] }, { userId: user?.id || null });
      return new Response(generated.stream, { headers: { "content-type": "text/event-stream; charset=utf-8", "cache-control": "no-store, no-transform", "x-accel-buffering": "no", "x-content-type-options": "nosniff", "x-shiguang-model-source": generated.provider } });
    }
    const generated = await completeWithFallback(env, { stream: false, temperature: 0.65, messages: [{ role: "system", content: system }, ...messages] }, { userId: user?.id || null, operation: input.mode === "mirror_result" ? "mirror_result" : "daily_guidance" });
...
async function relationshipReply(request, env) {
  if (request.method !== "POST") return new Response("Method not allowed", { status: 405 });
  if (!env.LLM_API_KEY || !env.LLM_MODEL) return json({ error: "llm_not_configured" }, 503);
  const input = await body(request);
  const messages = Array.isArray(input?.messages) ? input.messages.slice(0, 80).map((item) => ({ speaker: ["user", "other", "unknown"].includes(item?.speaker) ? item.speaker : "unknown", text: redactRelationshipSummary(item?.text, 1000) })).filter((item) => item.text) : [];
  const userNote = redactRelationshipSummary(input?.userNote, 2000);
  const analysis = redactRelationshipSummary(input?.analysis, 4000);
  ...
  try {
    ...
    const requested = redactRelationshipSummary(value?.recommendedReplyId, 40);
    return json({ options, recommendedReplyId: options.some((item) => item.id === requested) ? requested : options[0].id }, 200, { "cache-control": "no-store", "x-shiguang-model-source": generated.provider });
  } catch { return json({ error: "relationship_reply_failed" }, 502); }
}
