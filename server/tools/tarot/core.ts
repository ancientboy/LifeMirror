...
export type TarotCard = {
  ...
  provenance: {
    ...
    sourceEdition: "The Pictorial Key to the Tarot (1910)";
    sourceLocator: string;
    interpretationVersion: "tarot-rws-reference/1.1";
  };
};
...
export const TAROT_DECK: readonly TarotCard[] = [
  ...majors.map(([roman, name, upright, reversed], index) => ({
    ...
    reversed,
    provenance: {
      tradition: "Rider–Waite–Smith" as const,
      sourceEdition: "The Pictorial Key to the Tarot (1910)" as const,
      sourceLocator: `Part II · The Doctrine Behind the Veil · ${roman} ${name}`,
      interpretationVersion: "tarot-rws-reference/1.1" as const,
    },
  })),
  ...(
    ...
    ranks.map(([id, rank], index) => ({
      ...
      provenance: {
        tradition: "Rider–Waite–Smith" as const,
        sourceEdition: "The Pictorial Key to the Tarot (1910)" as const,
        sourceLocator: `Part III · The Outer Method of the Oracles · ${meta.name}${rank}`,
        interpretationVersion: "tarot-rws-reference/1.1" as const,
      },
    })),
  ),
...
/** Static report provenance. The professional layer is never generated by an LLM. */
export const TAROT_REFERENCE = {
  edition: "tarot-rws-reference/1.1",
  sources: [
    "Arthur Edward Waite, The Pictorial Key to the Tarot (1910)：大阿尔卡那与小阿尔卡那的 RWS 图像传统",
    "Pamela Colman Smith 的 Rider–Waite–Smith 牌面图像：图像象征与牌组结构参照",
    "现代反思式牌阵读法：牌位、正逆位、元素、数字与宫廷牌结构的固定组合规则",
  ],
} as const;
...
export function synthesizeTarotReading(question: string, spread: TarotSpread, cards: readonly DrawnCard[]): TarotReading {
  ...
  const cardInsights = cards.map((card, index) => {
    ...
    return {
      ...
      evidence: `${card.arcana === "major" ? "大阿尔卡那" : `${card.element}元素`} · ${position.prompt} · ${card.provenance.sourceLocator}`,
  ...
  return {
    ...
    sources: TAROT_REFERENCE.sources,
    method: `固定规则库 ${TAROT_REFERENCE.edition}：采用 Rider–Waite–Smith 图像传统的现代反思式读法；综合牌位、正逆位、元素、数字／宫廷结构与牌间反向证据。它提供观察假设，不提供确定性预测。`,
  };
}
