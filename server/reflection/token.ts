...
export function openReflectionDraft(token: string, secret: string): ReflectionDraftPayload {
  ...
  if ((payload.version !== 1 && payload.version !== 2 && payload.version !== 3 && payload.version !== 4 && payload.version !== 5 && payload.version !== 6 && payload.version !== 7) || new Date(payload.expiresAt).getTime() <= Date.now()) {
