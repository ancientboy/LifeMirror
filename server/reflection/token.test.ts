...
test("Context-aware v7 reflection drafts remain sealed and readable", () => {
  const token = sealReflectionDraft({ ...payload, version: 7 }, secret);
  assert.equal(openReflectionDraft(token, secret).version, 7);
});
